...
import sys
import json
from services.guoman8_scraper import Guoman8Scraper
...
def test_category_comics(category_id):
    ...
    scraper = Guoman8Scraper()
    url = f'{scraper.base_url}/list/smid-{category_id}'
    print(f"请求URL: {url}")
...
def test_hot_comics():
    ...
    scraper = Guoman8Scraper()
    print(f"请求URL: {scraper.base_url}/list/smid-1")
...
def test_search():
    ...
    keyword = "老师"
    scraper = Guoman8Scraper()
    search_url = f'{scraper.base_url}/search/{keyword}'
...
def test_chapter_images(chapter_id):
    ...
    if len(parts) == 2:
        ...
        image_url = f'{scraper.base_url}/{comic_id}/{chapter_num}.html'
    ...
    if result['total'] == 0:
        print("\n⚠️  未获取到图片URL，可能需要优化JavaScript解密逻辑")
    
    return True

def main():
    """主测试流程"""
    print("\n" + "🚀 开始测试国漫8采集源".center(60, "="))
    ...
    try:
        ...
        print("所有接口测试通过！")
        print("\n提示:")
        print("  - 如果图片数量为0，可能需要优化JavaScript解密逻辑")
