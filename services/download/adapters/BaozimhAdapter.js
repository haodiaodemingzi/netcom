...
export class BaozimhAdapter extends BaseAdapter {
  ...
  async getChapterImages(chapterId) {
    try {
      const response = await this.apiClient.get(`/chapters/${chapterId}/images`, {
        params: { source: 'baozimh' }
      });
      
      return this.normalizeImageData(response.data);
    } catch (error) {
      console.error('BaozimhAdapter获取图片失败:', error);
      throw error;
    }
  }
  ...
  async getChapterInfo(chapterId) {
    try {
      ...
      console.error('BaozimhAdapter获取章节信息失败:', error);
      throw error;
    }
  }
  ...
  async getDownloadHeaders() {
    // 获取baozimh的cookie
    const cookies = await this.downloadManager.getCookies('baozimh');
